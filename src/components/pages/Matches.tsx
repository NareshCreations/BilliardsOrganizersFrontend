...
selectedRoundDisplayName: '',
  ...
  private handleTournamentClick = async (tournamentId: string, tournamentName: string, organizerId?: string, venueId?: string): Promise<void> => {
    console.log('🏆 Tournament clicked:', { tournamentId, tournamentName, organizerId, venueId });
    ...
    try {
      ...
      // Call the new token-based tournament players API
      ...
      console.log('👥 Raw API response data:', JSON.stringify(playersResponse, null, 2));

      let registeredPlayers = [];
      let currentParticipants = 0;

      // Handle the new API response structure
      // Expected: { success: true, data: { tournamentId, players: [...], count: number } }
      if (playersResponse.success && playersResponse.data && playersResponse.data.players) {
        ...
        console.log('👥 Player data structure:', players.length > 0 ? players[0] : 'No players');
        ...
        console.log('👥 Player names:', registeredPlayers.map(p => p.name));
      } else {
        // No players returned by API
        ...
        console.warn('⚠️ Response structure:', {
          ...
          hasPlayers: !!(playersResponse.data?.players),
          playerCount: playersResponse.data?.count || 0
        });
        registeredPlayers = [];
        currentParticipants = 0;
      }
      ...
      console.log('🏆 Final player count:', tournamentData.currentParticipants);
      console.log('🏆 Player names:', tournamentData.registeredPlayers.map(p => p.name));
      ...
      const tournamentData = {
        ...
        maxParticipants: 50,
        currentParticipants: 0,
        description: `Tournament: ${tournamentName} (Error loading players)`,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        registeredPlayers: [],
        loading: false
              ...
              <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div className="space-y-4">
                  ...
                  {this.state.apiTournaments.length === 0 ? (
                    ...
                    return (
                    <div key={`api-${tournament.tournamentId}`} className="border border-gray-200 rounded-lg p-6 hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => this.handleTournamentClick(tournament.tournamentId, tournament.tournamentName, tournament.organizerId, tournament.venueId)}>
