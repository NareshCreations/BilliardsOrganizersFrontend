...
interface TournamentRound {
  ...
  byesCount?: number; // Number of byes in this round (bracket system only)
}

const BYE_PLAYER_ID = '00000000-0000-0000-0000-000000000001';
const PLACEHOLDER_PLAYER_ID = '00000000-0000-0000-0000-000000000002';
...
interface TournamentMatch {
  ...
  isBye?: boolean; // True if match has a bye
  positionInBracket?: number; // Position in bracket tree
  bracketSeed?: number; // Seed number in bracket
  nextMatchIdForWinner?: string; // Next match ID for winner advancement
}
...
interface MatchesState {
  ...
  showCancelMatchModal: boolean; // Controls cancel match modal visibility
  cancelMatchTarget: {
    matchId: string;
    roundId: string;
    roundApiId: string;
  ...
  cancelMatchSelectedPlayerId: string | null;
  cancelMatchModalLoading: boolean;
  cancelMatchModalError: string | null;
...
editingRoundName: string;
  ...
  // Bracket system state
  bracketType: 'manual' | 'power_of_2' | null; // Bracket system type
  bracketSize: number | null; // Power-of-2 bracket size (8, 16, 32, 64...)
  bracketData: any | null; // Full bracket structure from API
  isGeneratingBracket: boolean; // Loading state for bracket generation
  selectedPlayersForBracket: string[]; // Selected player IDs for bracket generation
  showBracketView: boolean; // Show bracket visualization
}

export class TournamentBracket extends BaseComponentComplete<TournamentBracketProps, MatchesState> {
  ...
  private queueBracketRefresh = (reason: string = 'socket-update'): void => {
    if (this.bracketRefreshTimeout) {
      window.clearTimeout(this.bracketRefreshTimeout);
    }
    this.bracketRefreshTimeout = window.setTimeout(() => {
      this.bracketRefreshTimeout = null;
      this.refreshBracketData(reason);
    }, 500);
  };

  private refreshBracketData = async (reason: string = 'manual'): Promise<void> => {
    const tournamentId = this.state.currentGameMatch?.tournamentId || this.state.selectedTournament?.id;
    if (!tournamentId) {
      return;
    }
  ...
  private getRealPlayerCount = (round: TournamentRound | null | undefined): number => {
    ...
    const collect = (player?: Player | null) => {
      if (!player) return;
      const baseId = player.id || player.customerProfileId || player.userId;
      if (!baseId) return;
      if (player.id === BYE_PLAYER_ID || player.id === PLACEHOLDER_PLAYER_ID) return;
      uniqueIds.add(baseId.toString());
    };

    (round.players || []).forEach(collect);
    (round.matches || []).forEach(match => {
      if (match.player1 && match.player1.id !== BYE_PLAYER_ID && match.player1.id !== PLACEHOLDER_PLAYER_ID) {
        collect(match.player1);
      }
      if (match.player2 && match.player2.id !== BYE_PLAYER_ID && match.player2.id !== PLACEHOLDER_PLAYER_ID) {
        collect(match.player2);
      }
    });
  ...
  private findMatchById(matchId: string): { match: TournamentMatch; round: TournamentRound } | null {
    ...
    if (this.state.currentGameMatch?.rounds) {
...
selectedRoundDisplayName: '',
      ...
      cancelMatchTarget: null,
      cancelMatchSelectedPlayerId: null,
      cancelMatchModalLoading: false,
      cancelMatchModalError: null,
      ...
      // Bracket system state
      bracketType: null, // Will be set when bracket is generated
      bracketSize: null,
      bracketData: null,
  ...
  componentDidUpdate(prevProps: TournamentBracketProps, prevState: MatchesState) {
   ...
   */
  private addAppNotification = (message: string, type: 'success' | 'error' | 'info' | 'warning' = 'info'): void => {
    // Simple notification implementation - can be enhanced later
    console.log(`[${type.toUpperCase()}] ${message}`);
    // For now, use the custom alert state
    this.setState({
      showCustomAlert: true,
      customAlertTitle: type === 'error' ? 'Error' : type === 'success' ? 'Success' : 'Notification',
  ...
  private handleMatchUpdate = (data: {
    ...
    if (this.state.currentGameMatch?.tournamentId === data.tournamentId) {
      if (data.deleted) {
        ...
        this.queueBracketRefresh('socket-match-deleted');
      } else {
        // Update match in UI
        this.setState(prevState => ({
          rounds: prevState.rounds.map(round => {
        ...
        if (data.status === 'completed') {
          ...
          this.queueBracketRefresh('socket-match-started');
        } else if (data.player1Id || data.player2Id) {
          this.queueBracketRefresh('socket-match-players-updated');
        }
      }
    }
  };
  ...
  private handleRoundUpdate = (data: {
    ...
    if (this.state.currentGameMatch?.tournamentId === data.tournamentId) {
      ...
      if (data.status === 'completed') {
        ...
        this.queueBracketRefresh('socket-round-completed');
        ...
        this.queueBracketRefresh('socket-round-started');
      }
    }
  };
  ...
  private handleTournamentClick = async (tournamentId: string, tournamentName: string, organizerId?: string, venueId?: string, tournamentStatus?: string): Promise<void> => {
    ...
    try {
      ...
      // Extract bracket information from tournament data
      let bracketType: 'manual' | 'power_of_2' | null = null;
      let bracketSize: number | null = null;
      let bracketGeneratedAt: string | null = null;

      // Check tournament_status object for bracket info (new API structure)
      if (playersResponse.data?.tournament_status?.tournament) {
        const tournamentData = playersResponse.data.tournament_status.tournament;
        bracketType = tournamentData.bracket_type || tournamentData.bracketType || null;
        bracketSize = tournamentData.bracket_size || tournamentData.bracketSize || null;
        bracketGeneratedAt = tournamentData.bracket_generated_at || tournamentData.bracketGeneratedAt || null;
        
        console.log('🎯 Extracted bracket info from tournament_status:', {
          bracketType,
          bracketSize,
          bracketGeneratedAt
        });
      ...
      // Also check direct tournament object if available
      if (playersResponse.data?.tournament) {
        const tournamentData = playersResponse.data.tournament;
        bracketType = bracketType || tournamentData.bracket_type || tournamentData.bracketType || null;
        bracketSize = bracketSize || tournamentData.bracket_size || tournamentData.bracketSize || null;
        bracketGeneratedAt = bracketGeneratedAt || tournamentData.bracket_generated_at || tournamentData.bracketGeneratedAt || null;
      }

      // Check bracket status if tournament is started/running and bracket info not found
      if (['started', 'running', 'ongoing'].includes(status) && !bracketType) {
        await this.getBracketStatus(tournamentId);
      }
  ...
  this.setState({
    ...
    selectedTournament: null,
    loading: false,
    // Set bracket information from extracted data
    bracketType: bracketType as 'manual' | 'power_of_2' | null,
    bracketSize: bracketSize,
    showBracketView: bracketType === 'power_of_2', // Show bracket view if bracket exists
    ...
    shuffledRounds: this.state.shuffledRounds || new Set<string>()
        ...
        this.setState({
          ...
          showGameOrganization: false,
          loading: false,
          // Set bracket information
          bracketType: bracketType as 'manual' | 'power_of_2' | null,
          bracketSize: bracketSize
        }, () => {
          console.log('✅ Popup shown with registered users (status:', status, ')');
          console.log('🎯 Bracket info:', { bracketType, bracketSize });
        });
  ...
  private handleNextGameOrganization = async (): Promise<void> => {
    ...
    try {
      ...
      if (startResponse.success) {
        ...
        // Check bracket status after starting tournament
        try {
          await this.getBracketStatus(tournamentId);
        } catch (error) {
          console.warn('⚠️ Could not get bracket status:', error);
        }
  ...
  private startMatch = async (matchId: string): Promise<void> => {
    ...
    const player1Id = foundMatch.player1.id || foundMatch.player1.customerProfileId || foundMatch.player1.userId;
    const player2Id = foundMatch.player2.id || foundMatch.player2.customerProfileId || foundMatch.player2.userId;
  ...
  private cancelMatch = async (matchId: string, giveByeTo?: string | null): Promise<void> => {
    try {
      ...
      if (response.success) {
        ...
        const advancedPlayer = giveByeTo
          ? [cancelledMatch.player1, cancelledMatch.player2].find(player => player && player.id === giveByeTo) || null
          : null;
        const playersToKeepInRound = [cancelledMatch.player1, cancelledMatch.player2].filter(player => {
          if (!player) return false;
          if (giveByeTo && player.id === giveByeTo) {
            return false;
          }
          return true;
        });
  ...
  private handleOpenCancelMatchModal = (matchId: string): void => {
    const matchData = this.findMatchById(matchId);
    if (!matchData) {
      this.showCustomAlert('Error', 'Match not found. Cannot cancel match.');
      return;
    }
    ...
    this.setState({
      ...
      cancelMatchTarget: {
        ...
        roundId: matchData.round.id,
        roundApiId: matchData.round.apiRoundId || matchData.round.id,
  ...
  private handleConfirmCancelMatch = async (): Promise<void> => {
    if (!this.state.cancelMatchTarget) {
      return;
    }

    this.setState({ cancelMatchModalLoading: true, cancelMatchModalError: null });
    try {
      await this.cancelMatch(this.state.cancelMatchTarget.matchId, this.state.cancelMatchSelectedPlayerId);
      this.setState({
        showCancelMatchModal: false,
        cancelMatchTarget: null,
        cancelMatchSelectedPlayerId: null,
  ...
  private selectWinner = async (winnerPlayer: Player): Promise<void> => {
    ...
    if (currentRound) {
      ...
      await this.refreshBracketData('complete-match-success');
    } else {
  ...
  private showDeleteConfirmModal = (title: string, message: string, callback: () => void): void => {
    this.setState({
  ...
  private hideDeleteConfirmModal = (): void => {
    this.setState({
  ...
  // ========== BRACKET SYSTEM METHODS ==========
  
  /**
   * Generate professional power-of-2 bracket
   * This is the ONE-TIME bracket generation that creates the full bracket structure
   */
  private handleGenerateBracket = async (): Promise<void> => {
    // Get tournament ID from currentGameMatch or selectedTournament
    const tournamentId = this.state.currentGameMatch?.tournamentId || 
                        (this.state.selectedTournament?.id as string);
    
    if (!tournamentId) {
      this.addAppNotification('Tournament ID not found', 'error');
      return;
    }

    // Get players from currentGameMatch (dashboard) or selectedTournament (modal)
    let players: any[] = [];
    if (this.state.currentGameMatch?.allPlayers) {
      // From dashboard - use allPlayers
      players = this.state.currentGameMatch.allPlayers;
    } else if (this.state.selectedTournament?.registeredPlayers) {
      // From modal - use registeredPlayers
      players = this.state.selectedTournament.registeredPlayers;
    }
    
    if (players.length < 2) {
      this.addAppNotification('Need at least 2 players to generate bracket', 'error');
      return;
    }

    // Get selected players - priority order:
    // 1. Players selected in dashboard (players with selected: true)
    // 2. Players selected in modal (selectedPlayersForBracket)
    // 3. All available players if none selected
    const selectedPlayers = this.state.currentGameMatch?.allPlayers?.filter((p: Player) => p.selected) || [];
    const playerIds = selectedPlayers.length > 0
      ? selectedPlayers.map(p => p.customerProfileId || p.userId || p.id).filter(Boolean) as string[]
      : (this.state.selectedPlayersForBracket.length > 0
          ? this.state.selectedPlayersForBracket
          : players.map((p: any) => p.customerProfileId || p.userId || p.id).filter(Boolean) as string[]);

    if (playerIds.length < 2) {
      this.addAppNotification('Please select at least 2 players', 'error');
      return;
    }

    try {
      this.setState({ isGeneratingBracket: true });

      console.log('🎯 Generating bracket for tournament:', tournamentId);
      console.log('👥 Selected players:', playerIds.length);

      const response = await matchesApiService.generateBracket(tournamentId, playerIds);

      if (response.success && response.data) {
        // Backend returns: bracketSize, rounds (or totalRounds), byes (or totalByes), matchesCreated
        const bracketSize = response.data.bracketSize || response.data.bracket_size;
        const totalRounds = response.data.rounds || response.data.totalRounds;
        const totalByes = response.data.byes || response.data.totalByes || 0;
        const matchesCreated = response.data.matchesCreated || response.data.matches_created || 0;
        const bracketType = response.data.bracketType || response.data.bracket_type || 'power_of_2';
        const bracketGeneratedAt = response.data.bracketGeneratedAt || response.data.bracket_generated_at;
        ...
        // If called from modal, close modal and navigate to dashboard
        if (this.state.selectedTournament && !this.state.currentGameMatch) {
          // Close modal and reload tournament to show bracket
          this.setState({
            showTournamentModal: false,
            selectedTournament: null
          });
          
          // Reload tournament data to show updated bracket status
          // The user can click on tournament again to see the bracket
        }

        this.addAppNotification(
          `Bracket generated successfully! ${totalByes} player(s) received byes in Round 1. Total matches: ${matchesCreated}.`,
          'success'
        );
      } else {
        throw new Error(response.message || 'Failed to generate bracket');
      }
    } catch (error) {
      console.error('❌ Error generating bracket:', error);
      this.setState({ isGeneratingBracket: false });
      ...
      if (error instanceof Error) {
        ...
        // Check for "Cannot POST" error (backend route not found)
        if (errorMessage.includes('Cannot POST') || errorMessage.includes('404')) {
          errorMessage = 'Bracket generation endpoint not found. Please ensure the backend server has the bracket generation route configured at POST /api/v1/organizers/tournaments/:tournamentId/generate-bracket';
        } else if (errorMessage.includes('401') || errorMessage.includes('Unauthorized')) {
          errorMessage = 'Session expired. Please login again.';
        } else if (errorMessage.includes('403') || errorMessage.includes('Forbidden')) {
          errorMessage = 'You do not have permission to generate bracket for this tournament.';
        } else if (errorMessage.includes('400') || errorMessage.includes('Bad Request')) {
          errorMessage = 'Invalid request. Please check that you have selected at least 2 players.';
        }
      }
      
      this.addAppNotification(errorMessage, 'error');
    }
  };
   ...
   */
  private loadBracket = async (tournamentId: string): Promise<void> => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const bracketSize = tournament?.bracketSize || tournament?.bracket_size;
        
        if (bracketType === 'power_of_2') {
          this.setState({
            bracketType: 'power_of_2' as const,
            bracketSize: bracketSize || null,
            bracketData: response.data,
            showBracketView: true
          });
          ...
          const convertedRounds: TournamentRound[] = apiRounds.map((round: any) => {
            ...
            const roundDisplayName = round.roundDisplayName || round.displayName || roundName;
            const roundNumber = round.roundNumber || round.round_number || 1;
            ...
            return {
              ...
              completedMatches: roundMatches.filter((m: any) => 
          ...
          this.setState(prevState => ({
            ...
            activeRoundSubTab: firstRoundId
              ? {
                  ...prevState.activeRoundSubTab,
                  [firstRoundId]: prevState.activeRoundSubTab[firstRoundId] || 'players'
                }
              : prevState.activeRoundSubTab
          }));
   ...
   * Get bracket status
   */
  private getBracketStatus = async (tournamentId: string): Promise<void> => {
    try {
      const response = await matchesApiService.getBracketStatus(tournamentId);
      
      if (response.success && response.data) {
        // Backend returns camelCase: bracketType, bracketSize, bracketGeneratedAt
        // Also handle snake_case format for compatibility
        const bracketType = response.data.bracketType || response.data.bracket_type;
        const bracketSize = response.data.bracketSize || response.data.bracket_size;
        const bracketGeneratedAt = response.data.bracketGeneratedAt || response.data.bracket_generated_at;
        const hasBracket = response.data.hasBracket !== undefined ? response.data.hasBracket : (bracketType !== null && bracketType !== undefined);
        
        if (hasBracket && bracketType === 'power_of_2') {
          this.setState({
            bracketType: 'power_of_2' as const,
            bracketSize: bracketSize || null,
            showBracketView: true
          ...
          // Load full bracket if it exists
          await this.loadBracket(tournamentId);
        }
      }
    } catch (error) {
  ...
  /**
   * Delete bracket (revert to manual system)
   */
  private handleDeleteBracket = async (): Promise<void> => {
    if (!this.state.selectedTournament) {
      this.addAppNotification('No tournament selected', 'error');
      return;
    }
    ...
    this.showDeleteConfirmModal(
      'Delete Bracket',
      'Are you sure you want to delete the bracket? This will revert the tournament to manual mode. This action cannot be undone.',
      async () => {
        try {
          const response = await matchesApiService.deleteBracket(tournamentId);
          ...
          if (response.success) {
            ...
            this.addAppNotification('Bracket deleted successfully. Tournament reverted to manual mode.', 'success');
          } else {
            throw new Error(response.message || 'Failed to delete bracket');
          }
        } catch (error) {
   ...
   * Toggle bracket view
   */
  private toggleBracketView = (): void => {
    this.setState({ showBracketView: !this.state.showBracketView });
  };

  /**
   * Calculate bracket parameters (power-of-2)
   */
  private calculateBracketParams = (totalPlayers: number): {
    bracketSize: number;
    byes: number;
    rounds: number;
  } => {
 ...
 <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-4">
                    <div className="bg-gray-50 p-4 rounded-lg">
                      ...
                      {/* Generate Bracket Button - Show only if tournament is started and bracket not generated */}
                      {this.state.currentGameMatch?.tournamentStarted && 
                       !this.state.bracketType && 
                       (this.state.currentGameMatch?.status === 'started' || this.state.currentGameMatch?.status === 'running' || this.state.currentGameMatch?.status === 'active') && (
                        <button
                          onClick={this.handleGenerateBracket}
                          disabled={this.state.isGeneratingBracket || 
                                   ((this.state.currentGameMatch?.allPlayers?.filter((p: Player) => p.selected).length || 0) === 0 &&
                                    (this.state.currentGameMatch?.allPlayers?.length || 0) < 2)}
                          className="w-full mb-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-3 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                          title={
                            (this.state.currentGameMatch?.allPlayers?.filter((p: Player) => p.selected).length || 0) === 0 &&
                            (this.state.currentGameMatch?.allPlayers?.length || 0) < 2
                              ? 'Need at least 2 players to generate bracket'
                              : (this.state.currentGameMatch?.allPlayers?.filter((p: Player) => p.selected).length || 0) > 0
                                ? `Generate bracket with ${this.state.currentGameMatch?.allPlayers?.filter((p: Player) => p.selected).length} selected player(s)`
                                : 'Generate bracket with all players'
                          }
                        >
                          {this.state.isGeneratingBracket ? (
                            <>
                      ...
                      {/* View Bracket / Delete Bracket Buttons - Show if bracket exists */}
                      {this.state.bracketType === 'power_of_2' && (
                        <>
                          <button
                            onClick={this.toggleBracketView}
                            className="w-full mb-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-4 py-3 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all font-medium flex items-center justify-center gap-2"
                          >
                            <Trophy className="w-5 h-5" />
                            {this.state.showBracketView ? 'Hide Bracket' : 'View Bracket'}
                          </button>
                          <button
                            onClick={() => {
                              if (this.state.currentGameMatch?.tournamentId) {
                                this.handleDeleteBracket(this.state.currentGameMatch.tournamentId, this.state.currentGameMatch.name);
                              }
                            }}
                            ...
                            title="Delete bracket and revert to manual tournament"
                          >
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
   ...
   {/* Round Selection with Stats */}
    ...
    <div className="space-y-2">
      ...
      {/* Bracket System Info */}
      {this.state.bracketType === 'power_of_2' && this.state.bracketSize && (
        <div className="mt-3 pt-3 border-t border-gray-200">
          <div className="flex items-center gap-2 mb-2">
            <Trophy className="w-4 h-4 text-yellow-600" />
            <span className="text-gray-700 font-medium">Bracket System Active</span>
          </div>
          <div className="pl-6 space-y-1 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-600">Bracket Size:</span>
              <span className="font-medium text-blue-600">{this.state.bracketSize} players</span>
            </div>
            {this.state.bracketData && (
              <>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total Rounds:</span>
                  <span className="font-medium text-purple-600">{this.state.bracketData.totalRounds || Math.log2(this.state.bracketSize)}</span>
                </div>
                {this.state.bracketData.totalByes > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-600">Byes (Round 1):</span>
                    <span className="font-medium text-orange-600">{this.state.bracketData.totalByes}</span>
                  </div>
                )}
              </>
            )}
                  ...
                  {this.state.activeLobbyTab === 'winners' && (
                    <div>
                      ...
                      {this.state.currentGameMatch.rounds?.map((round: TournamentRound) => {
                        const roundWinners =
                          this.state.currentGameMatch?.allPlayers?.filter(
                            (p) => p.roundsWon && p.roundsWon.includes(round.name)
                          ) || [];
                        ...
                        return (
                          <div
                            key={round.id}
                            className="mb-6 bg-gradient-to-r from-green-50 to-blue-50 rounded-xl p-4 border border-green-200"
                          >
                            <div className="flex items-center justify-between mb-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                              {roundWinners.map((player: Player) => (
                                <div
                                  key={player.id}
                                  className="bg-white p-3 rounded-lg border-2 border-green-300 shadow-sm relative"
                                >
                                  <div className="absolute top-2 right-2">
              ...
              <div className="p-6">
                ...
                <div className="bg-white border border-gray-200 rounded-lg p-6 mb-8">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      {/* Bracket Generation Button - Only show if tournament is started and bracket not generated */}
                      {this.state.selectedTournament.status === 'started' || this.state.selectedTournament.status === 'running' ? (
                        !this.state.bracketType ? (
                          <button 
                            onClick={this.handleGenerateBracket}
                            disabled={this.state.isGeneratingBracket || this.state.selectedTournament.loading || 
                                     (this.state.selectedPlayersForBracket.length === 0 && 
                                      (this.state.selectedTournament.registeredPlayers?.length || 0) < 2)}
                            className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-3 rounded-lg hover:from-blue-700 hover:to-indigo-700 transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
                            title={
                              this.state.selectedPlayersForBracket.length === 0 && 
                              (this.state.selectedTournament.registeredPlayers?.length || 0) < 2
                                ? 'Need at least 2 players to generate bracket'
                                : this.state.selectedPlayersForBracket.length > 0
                                  ? `Generate bracket with ${this.state.selectedPlayersForBracket.length} selected player(s)`
                                  : 'Generate bracket with all players'
                            }
                          >
                            {this.state.isGeneratingBracket ? (
                              ...
                              <>
                                ...
                                {this.state.selectedPlayersForBracket.length > 0 && (
                                  <span className="ml-1 px-2 py-0.5 bg-white/20 rounded text-xs">
                                    ({this.state.selectedPlayersForBracket.length})
                                  </span>
                                )}
                              </>
                            )}
                          ...
                          <div className="flex items-center gap-3">
                            <span className="px-4 py-2 bg-green-100 text-green-800 rounded-lg font-medium text-sm">
                              Bracket: {this.state.bracketSize}-Player
                            </span>
                            <button 
                              onClick={this.toggleBracketView}
                            ...
                            <button 
                              onClick={this.handleDeleteBracket}
                      ...
                      <button 
                        onClick={() => this.handleShuffleGame()}
                        disabled={this.state.selectedTournament.loading || this.state.bracketType === 'power_of_2'}
                        ...
                        title={this.state.bracketType === 'power_of_2' ? 'Bracket system is active. Shuffle is disabled.' : ''}
                      >
                        Shuffle Game
                      </button>
                    </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-bold text-gray-900">Registered Players ({this.state.selectedTournament.currentParticipants})</h3>
                    
                    {/* Player Selection Controls - Show if tournament is started and bracket not generated */}
                    {(this.state.selectedTournament.status === 'started' || this.state.selectedTournament.status === 'running') && 
                     !this.state.bracketType && (
                      <div className="flex items-center gap-3">
                        <button
                          onClick={() => {
                            const allPlayerIds = (this.state.selectedTournament.registeredPlayers || [])
                              .map((p: any) => p.customerProfileId || p.userId || p.id)
                              .filter(Boolean);
                            this.setState({ selectedPlayersForBracket: allPlayerIds });
                          }}
                        ...
                        <button
                          onClick={() => this.setState({ selectedPlayersForBracket: [] })}
                          className="px-4 py-2 bg-gray-600 text-white text-sm rounded-lg hover:bg-gray-700 transition-colors"
                        >
                          Deselect All
                        </button>
                        <span className="text-sm text-gray-600">
                          Selected: {this.state.selectedPlayersForBracket.length} / {this.state.selectedTournament.currentParticipants}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {this.state.selectedTournament.loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {this.state.selectedTournament.registeredPlayers.map((player: any) => {
                        const playerId = player.customerProfileId || player.userId || player.id;
                        const isSelected = this.state.selectedPlayersForBracket.includes(playerId);
                        const canSelect = (this.state.selectedTournament.status === 'started' || this.state.selectedTournament.status === 'running') && !this.state.bracketType;
                        
                        return (
                          <div 
                            key={player.id} 
                            className={`bg-white border rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer ${
                              canSelect && isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200'
                            }`}
                            onClick={() => {
                              if (canSelect) {
                                const playerId = player.customerProfileId || player.userId || player.id;
                                if (isSelected) {
                                  this.setState({
                                    selectedPlayersForBracket: this.state.selectedPlayersForBracket.filter(id => id !== playerId)
                                  });
                                } else {
                                  this.setState({
                                    selectedPlayersForBracket: [...this.state.selectedPlayersForBracket, playerId]
                                  });
                                }
                              }
                            }}
                            ...
                            <div className="flex items-start gap-3">
                              {canSelect && (
                                <input
                                  ...
                                  className="mt-2 w-5 h-5 text-blue-600 rounded focus:ring-blue-500 cursor-pointer"
                                />
                              )}
                ...
                <div className="mt-8 flex items-center justify-center gap-4">
                  {/* Start Tournament Button - Show if tournament is not started */}
                  {(this.state.selectedTournament.status === 'pending' || 
                    this.state.selectedTournament.status === 'scheduled' || 
                    this.state.selectedTournament.status === 'registration_open') && (
                    <button 
                      onClick={() => this.handleNextGameOrganization()}
                      disabled={this.state.selectedTournament.loading}
                      className="bg-gradient-to-r from-green-600 to-blue-600 text-white px-8 py-4 rounded-lg hover:from-green-700 hover:to-blue-700 transition-all font-medium text-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      Start Tournament
                  ...
                  {/* Organize Tournament Button - Show if tournament is started/running */}
                  {(this.state.selectedTournament.status === 'started' || 
                    this.state.selectedTournament.status === 'running') && !this.state.bracketType && (
                    <button 
                      onClick={() => this.handleNextGameOrganization()}
                      disabled={this.state.selectedTournament.loading}
                      className="bg-gradient-to-r from-green-600 to-blue-600 text-white px-8 py-4 rounded-lg hover:from-green-700 hover:to-blue-700 transition-all font-medium text-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                  ...
                  {/* View Bracket Button - Show if bracket exists */}
                  {this.state.bracketType === 'power_of_2' && (
                    <button 
                      onClick={() => {
                        this.setState({
                        ...
                        // Load bracket when viewing
                        if (this.state.selectedTournament?.id) {
                          this.loadBracket(this.state.selectedTournament.id);
                        }
                      }}
                      className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-8 py-4 rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all font-medium text-lg flex items-center gap-2"
                    >
                      <Trophy className="w-5 h-5" />
                      View Bracket
                    </button>
...
<div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 transform transition-all">
      ...
      {/* Custom Alert Modal */}
      {this.state.showCancelMatchModal && this.state.cancelMatchTarget && (() => {
        const match = this.state.cancelMatchTarget.match;
        const players = [match.player1, match.player2].filter(Boolean) as Player[];
        const eligiblePlayers = players.filter(
          player => player.id !== BYE_PLAYER_ID && player.id !== PLACEHOLDER_PLAYER_ID
        );
        const selectedPlayerId = this.state.cancelMatchSelectedPlayerId;
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 px-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 relative">
              <button
                ...
                aria-label="Close cancel match modal"
                type="button"
              >
                ✕
              </button>

              <h3 className="text-2xl font-bold text-gray-900 mb-2">Cancel Match</h3>
              <p className="text-sm text-gray-600 mb-5">
                Choose a player to award a bye (auto-advance) or remove both players from this match.
                Advancing a player will immediately fill the next round slot via the backend.
              </p>
              ...
              <div className="space-y-4 max-h-60 overflow-y-auto pr-1">
                {eligiblePlayers.length > 0 ? (
                  eligiblePlayers.map(player => (
                    <label
                      key={player.id}
                      className={`flex items-center space-x-3 border rounded-xl px-4 py-3 cursor-pointer transition ${
                    ...
                    >
                      <input
                        type="radio"
                        name="cancel-match-bye-selection"
                        value={player.id}
                        checked={selectedPlayerId === player.id}
                        onChange={() => this.setState({ cancelMatchSelectedPlayerId: player.id || null })}
                        className="text-blue-600 focus:ring-blue-500"
                      />
                      <div className="flex items-center space-x-3">
                        <img
                          src={getAvatar(player.avatar || player.profilePic)}
                          alt={this.getPlayerDisplayName(player)}
                  ...
                  <div className="border border-dashed border-gray-300 rounded-xl px-4 py-6 text-center text-sm text-gray-500">
                    No eligible players to advance. Cancelling will keep the next round slot as TBD.
                  </div>
                )}
                ...
                >
                  <input
                    ...
                    name="cancel-match-bye-selection"
                    value="none"
                    checked={selectedPlayerId === null}
                    onChange={() => this.setState({ cancelMatchSelectedPlayerId: null })}
                    className="text-gray-700 focus:ring-gray-500"
                  />
                  <div>
                    <p className="font-semibold text-gray-900">Remove both players</p>
                    <p className="text-xs text-gray-500">
                      Both players leave this spot. Next round stays TBD until someone advances into it.
                    </p>
                  </div>
                </label>
              </div>

              {this.state.cancelMatchModalError && (
              ...
              <div className="mt-6 flex items-center justify-end space-x-3">
                ...
                <button
                  className={`px-5 py-2 text-sm font-semibold text-white rounded-lg shadow ${
                    this.state.cancelMatchModalLoading
                      ? 'bg-blue-300 cursor-not-allowed'
                      : 'bg-blue-600 hover:bg-blue-700'
                  }`}
                  onClick={this.handleConfirmCancelMatch}
                  type="button"
                  disabled={this.state.cancelMatchModalLoading}
                >
                  {this.state.cancelMatchModalLoading ? 'Processing...' : 'Confirm'}
                </button>
              </div>
            </div>
          </div>
    ...
    <div className="border-b border-gray-200 overflow-x-auto">
      <nav className="-mb-px flex flex-nowrap space-x-6 pb-2">
        {this.state.rounds.map((round, index) => {
          const roundPlayerCount = this.getRealPlayerCount(round);
          return (
...
<div className="flex border border-gray-200 rounded-lg overflow-hidden bg-gray-50 shadow-sm">
  {/* Main Round Tab */}
          <button
            ...
            className={`py-3 px-5 font-semibold text-base flex items-center transition-all ${
...
<div className="mt-6 bg-white rounded-lg shadow-md p-6">
  {(() => {
    ...
    const realActivePlayerCount = this.getRealPlayerCount(activeRound);
...
{this.state.rounds
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-4">
            <span className={`px-4 py-2 rounded-full text-sm font-medium shadow-sm ${
              realActivePlayerCount === 0 
                ? 'bg-gray-500 text-white' 
                : realActivePlayerCount % 2 === 0 
                  ? 'bg-blue-500 text-white' 
                  : 'bg-red-500 text-white'
            }`}>
              {realActivePlayerCount} Player{realActivePlayerCount !== 1 ? 's' : ''}
              {realActivePlayerCount > 0 && realActivePlayerCount % 2 !== 0 && (
                    ...
                    <optgroup label="Move to Other Rounds">
                      {this.state.rounds
                        .filter(round => {
                          ...
                          const currentRoundHasCompletedMatches = activeRound.matches?.some(match => match.status === 'completed');
                        ...
                        .map(round => {
                          ...
                          const afterCount = currentCount + selectedCount;
                          const willBeEven = afterCount % 2 === 0;
                          const hasCompletedMatches = round.matches?.some(match => match.status === 'completed');
                          const isEmpty = round.players.length === 0 && round.winners.length === 0 && !hasCompletedMatches;
                          const isEmptyText = isEmpty ? ' (Empty)' : '';
                          return (
                            <option key={round.id} value={round.id}>
                              {round.displayName || round.name}
                              {isEmptyText} ({currentCount} → {afterCount} players) {willBeEven ? '✓' : '⚠️'}
                            </option>
                          );
                        })}
                    </optgroup>
  ...
  console.log('🔍 Shuffle button visibility check:', {
    ...
    unmatchedCount,
    playersCount: realActivePlayerCount,
  ...
  return null;
    ...
    const unmatchedPlayers = activeRound.players.filter(player => 
        !matchedPlayerIds.has(player.id) &&
        player.id !== BYE_PLAYER_ID &&
        player.id !== PLACEHOLDER_PLAYER_ID
      );
          ...
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-yellow-800 text-sm font-medium">
                This round has {realActivePlayerCount} players (odd number). Add or remove 1 player to create proper match pairings.
              </span>
            </div>
          </div>
        )}
        ...
        {activeRound.matches.map((match, index) => (
          <div key={match.id} className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            ...
            {(() => {
              const isByeMatch =
                match.isBye ||
                match.player1?.id === BYE_PLAYER_ID ||
                match.player2?.id === BYE_PLAYER_ID;
              const hasPlaceholderOpponent =
                match.player1?.id === PLACEHOLDER_PLAYER_ID ||
                match.player2?.id === PLACEHOLDER_PLAYER_ID;
              const awaitingOpponent = hasPlaceholderOpponent && !isByeMatch;
              return (
                <div className="mt-4 pt-4 border-t border-gray-200 flex justify-center space-x-3">
                  {match.status === 'pending' && !isByeMatch && !awaitingOpponent && (
                    <>
                      <button 
                      ...
                      <button 
                        onClick={() => this.handleOpenCancelMatchModal(match.id)}
                  ...
                  {match.status === 'active' && !isByeMatch && !awaitingOpponent && (
                    <>
                      <button 
                  ...
                  {match.status === 'completed' && (
                    <div className="text-center space-y-2">
                      <div className="px-4 py-2 bg-green-100 text-green-800 rounded-lg text-sm font-medium">
                        {isByeMatch
                          ? 'BYE • Player auto-advanced'
                          : awaitingOpponent
                            ? 'Awaiting opponent placement'
                            : '✅ Match Completed'}
                      </div>
                      {!isByeMatch && !awaitingOpponent && (
                        <button 
